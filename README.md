...
generator and diffed in CI like every text chart below:

![Horizontal grouped bars as an SVG terminal card](examples/speedup.svg)
...
- **Every host that draws a cell grid is a terminal.** `Plot::to_html` and
  ...
  ```rust
  :dep malevich = { version = "1.21", features = ["evcxr"] }
