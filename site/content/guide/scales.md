...
The differentiators live exactly where everyone else got bored. A scale maps a data domain to a raster range with the d3-scale contract: `nice`, `ticks(n)`, `invert`, a formatter. The axis it draws is treated as the product. Ticks are computed by the placement the visualization literature settled on. Labels are exact decimals that parse back to their values. If the axes are wrong, nothing else matters ([why](../../principles/axes-are-the-product/)).
...
{{pair scale_domain_auto scale_domain_fixed | Automatic and fixed. What falls outside a fixed domain is clipped. It is drawn nowhere. It does not smear the point onto the border. A point at the edge is a point at the edge.}}

{{figure scale_one_sided}}

Interactively, a `Viewport` is the same two windows as a value a host can zoom, pan, clamp, and tail. That is why zooming into millions of points needs no special machinery. The reduction re-aggregates to the new window ([interaction](../interaction/)).
