...
Two tiny required dependencies (`terminal_size`, `unicode-width`), no build script, `#![forbid(unsafe_code)]`. The crate is 1.x: the public API follows semver, guarded in CI by `cargo-semver-checks` against the last published release. Features are opt-in and named later on this page.
...
That is the whole program. `line` is a preset: a plain function that returns a `Plot`, and a `Plot` implements `Display` by detecting a frame for stdout — its width, a third of its height, whether it has color, which glyphs are safe. Print it into a pipe and you get clean text with no escape byte in it.
...
Missing data is `NaN`. It renders as a visible break, never interpolated across, never dropped silently. This is the de-facto convention of the terminal plotting field, and here it is universal: every stat, every mark, every axis speaks it.

{{figure start_gap}}

## Taking the lid off

Every preset is a name for a grammar expansion. `line(values)` is `Plot::new().layer(Line::y(values))`, and a test asserts the two render byte for byte the same. So when the preset is not enough — a second series, a reference line, a title — you do not switch libraries or APIs; you write the same plot with the lid off.
...
The same renderer, from any shell: [`kaz`](../../cli/) is a stdin-first plotter, one subcommand per chart, plot on stderr so data can flow on through stdout.
...
And the same engine compiled to wasm, on [npm](../../js/), with an Ink widget that shares the ratatui adapter's interaction grammar:
