...
Live data machinery is kept at the edge of the crate, in `malevich::stream`, and it is small on purpose: a sliding window shared across threads, a counter-to-rate helper, and an in-place repaint that never owns the screen. The core stays pure — every render that runs is still a pure function of its inputs — and time enters only at this rim.

## The window

`Ring::new(capacity)` is a sliding window: push readings from a sampler thread, snapshot from the UI thread, and render the snapshot. It is the one lock in the library. `Ring::growing` keeps every value since the start instead of a fixed window. `Rate` turns a monotonic counter — bytes received, requests served — into per-interval deltas.
...
For a window over a series that keeps growing, the `Viewport` has `tail(latest, width)`: a window ending at the latest x, `width` wide. The reduction re-aggregates to it on every frame, so a ten-million-point history tails at full speed.
...
No animations, no event loop, no raw mode. The library writes one frame when you ask it to and never reads input. A host that wants a schedule owns the loop; a host that wants gestures owns the mouse ([interaction](../interaction/)). That boundary is what lets the same plot value render in a ratatui dashboard, an Ink app, a notebook, and a CI log without a mode switch.
